...
Maintainer runbook for shipping HOOK across npm, GitHub, and the website-facing changelog surface.
...
HOOK uses calendar versions that still fit semver syntax:
...
2. **npm** — `hookai` and public workspace packages are published
...
# or
npx hookai@canary onboard --data-dir "$(mktemp -d /tmp/hook-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/hookai/hook/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for HOOK. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
gh workflow run release-smoke.yml -f hook_version=canary
gh workflow run release-smoke.yml -f hook_version=latest
```

Minimum checks:

- `npx hookai@canary onboard` installs
