...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented HOOK plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside HOOK.
...
- package: `@hookai/plugin-kitchen-sink-example`
- plugin id: `hook.kitchen-sink-example` or `hook-kitchen-sink-example`

Recommendation: use `hook-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core HOOK event such as `issue.created`
...
Recommendation: default to a HOOK-local endpoint or a stable public echo endpoint to avoid flaky docs.
