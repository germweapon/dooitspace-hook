...
HOOK already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model HOOK should implement next.

## Product Goals

HOOK should let operators:
...
For the next implementation pass, HOOK should enforce these defaults:
...
HOOK should separate subscription-included usage from billed spend:
...
HOOK should have two threshold classes:
...
HOOK must know that a project is budget-paused and enforce it.
...
HOOK must also block execution before new work starts.
