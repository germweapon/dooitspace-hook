...
Add a single storage subsystem for HOOK that supports:
...
- API serves attachment bytes through authenticated HOOK endpoints.
...
- Default local path is under instance root: `~/.hook/instances/<instanceId>/data/storage`.
...
- `hookai onboard` writes a valid `storage` config block by default.
- `hookai configure --section storage` can switch between local and s3 modes.
...
- `hookai doctor` reports actionable storage status.
