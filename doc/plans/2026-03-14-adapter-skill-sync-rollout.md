...
This document defines the rollout plan for adapter-wide skill support in HOOK.
...
- HOOK stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

HOOK currently has these adapters:
...
1. Desired skills live in HOOK for every adapter.
...
These adapters have a stable local skills directory that HOOK can read and manage.
...
These adapters do not have a meaningful HOOK-owned persistent install state.
...
- runtime already injects HOOK skills into `~/.cursor/skills`
...
- runtime already injects HOOK skills into `~/.gemini/skills`
...
- runtime already injects HOOK skills into `~/.pi/agent/skills`
...
- HOOK can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects HOOK skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- HOOK may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed HOOK skills that are clearly marked as HOOK-managed
...
- HOOK stores desired skills only
...
- avoid destructive wording unless HOOK can prove a skill is HOOK-managed
...
That gets HOOK from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
