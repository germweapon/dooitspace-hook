...
Agents need API keys to authenticate with HOOK. The current approach
...
**Trust model:** The adapter process runs on the same machine as the HOOK
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** HOOK generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a HOOK CLI
...
```
hook auth login
# Opens browser -> user confirms -> token stored at ~/.hook/credentials
```
...
managed by the HOOK server itself.
...
1. A company admin (user or agent) generates an **invite URL** from HOOK.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The HOOK SKILL.md (or a link to it)
   - What information HOOK needs from the agent (e.g. webhook URL, adapter
...
5. HOOK stores the pending registration.
...
7. On approval, HOOK provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** HOOK issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.hook.ing/skills/hook/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How HOOK should send you heartbeats",
...
1. Generate a HOOK invite link for the company.
...
4. A HOOK company member approves the new agent.
5. HOOK begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `hookai auth login` for developer-managed remote agents.                                      |
