...
`PAP-447` asks how HOOK should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same HOOK instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into HOOK:

- some operators use HOOK to manage HOOK and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as HOOK
- Claude and Codex expose different built-in affordances, so HOOK should not overfit to one tool

## Core Product Decision

HOOK should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by HOOK
...
- HOOK should not assume that every adapter can see or use a host filesystem path directly
...
If HOOK makes worktrees agent-first, it will blur:
...
- HOOK core resolves and records execution workspace state
...
That gives HOOK room to support:
...
- HOOK handles workspace runtime services under the hood using project/workspace policy
...
Once HOOK is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that HOOK cannot infer
...
- when issue moves to done, should HOOK auto-commit?
- should it auto-push?
...
HOOK already has the right foundation for a project-first model.
...
For server-spawned local adapters, HOOK already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that HOOK should mirror directly

Implication:

- for `codex_local`, HOOK should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but HOOK should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, HOOK should separate:
...
HOOK should be able to express intentions such as:
...
These are not specific to HOOK. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".hook/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a HOOK-specific `devServer` flag.
...
HOOK should distinguish between:
...
HOOK should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, HOOK should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to HOOK
...
For runtime services specifically, HOOK should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as HOOK-only dev-server flags
...
3. Inject resolved execution workspace details into `context.hookWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / HOOK instance can be reused through env/config injection when appropriate
...
- HOOK can decide whether to reuse or start a fresh service deterministically
...
3. Test against HOOK-on-HOOK workflows first.
...
7. Workspace runtime services are modeled generically, not as HOOK-specific dev-server toggles.
