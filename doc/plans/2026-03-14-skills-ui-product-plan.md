...
This document defines the product and UI plan for skill management in HOOK.
...
- HOOK company import/export can include skills as package content
...
3. The UI must always tell the truth about what HOOK knows:
   - desired state in HOOK
...
HOOK should treat skills at two scopes:
...
- created locally inside HOOK later
...
7. “Understand what HOOK knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a HOOK-specific package layout
...
This matters because Codex and similar adapters may already have local skills that HOOK did not install.
...
- `desired_only`: HOOK wants it, adapter does not show it yet
- `external`: adapter has it but HOOK does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, HOOK only tracks desired state
...
- HOOK can still track desired skills
...
- `.hook.yaml` may add HOOK-specific fidelity, but should not replace the base shortname association model
...
- `.hook.yaml` may add HOOK-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
HOOK should have a company-scoped skill model or managed package model representing:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too HOOK-specific.
...
That gives HOOK one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
