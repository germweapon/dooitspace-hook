...
HOOK needs to support two very different but equally valid ways of working:
...
Today, HOOK already has the beginnings of this model:
...
- making HOOK feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing HOOK users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted HOOK deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning HOOK into a full code review product
...
This object must be recorded explicitly so that HOOK can:
...
HOOK should treat PRs as a type of work product linked back to:
...
HOOK already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted HOOK must be a first-class deployment mode

HOOK cannot assume that it is running on the same machine as the code.

In cloud deployments, HOOK may:
...
- `Runtime service`: a process or service HOOK owns or tracks for a workspace
...
It also lets cloud-hosted HOOK deployments point at codebases and remotes without pretending the HOOK host has direct filesystem access.
...
Without an explicit `execution workspace` record, HOOK has nowhere stable to attach:
...
HOOK needs a single place to show:
...
- HOOK-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without HOOK owning the execution host
...
## 1. HOOK host and execution host must be decoupled
...
without the HOOK host itself running local git or local preview processes.
...
HOOK should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for HOOK.
...
HOOK should keep workspace automation centered on a small set of hooks:
...
HOOK should provide the equivalent visibility in the project `Code` surface:
...
HOOK should treat execution workspaces as the natural home for:
...
HOOK should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
HOOK should continue treating cleanup policy as part of the core workspace model:
...
HOOK should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the HOOK host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

HOOK should borrow Worktrunk's good execution patterns while keeping the broader HOOK model:
...
That keeps HOOK focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted HOOK deployments remain first-class

That is a better fit for HOOK than either extreme:
