...
This document defines the next-stage plan for HOOK company import/export.

The core shift is:

- move from a HOOK-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside HOOK.
...
HOOK already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just HOOK
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `hook.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.hook.yaml` sidecar for high-fidelity HOOK-specific details
...
11. adapter-aware skill sync surfaces so HOOK can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point HOOK at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a HOOK sidecar.
...
HOOK must not redefine `SKILL.md`.
...
- HOOK-specific fidelity lives in `.hook.yaml`
- HOOK may resolve and install `SKILL.md` packages, but it must not require a HOOK-only skill format
...
### 5.4 Base Package Vs HOOK Extension
...
- base package:
  ...
  - no HOOK-only runtime fields by default
- HOOK extension:
  - `.hook.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by HOOK tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`hook.manifest.json` is not part of the future package direction.
...
HOOK import/export should support these entity kinds:
...
- HOOK can treat “this set of agents came from team package X” as the imported-team model
...
HOOK should support source references in package metadata with:
...
HOOK should define a new adapter surface area around skills:
...
Planned HOOK behavior:

- if an adapter supports read, HOOK should show current skills in the UI
- if an adapter supports write, HOOK should let the user enable/disable imported skills
- if an adapter supports sync, HOOK should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.hook.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.hook.yaml` alongside the base package
...
HOOK should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `hookai company export <company-id> --out <path>`
- `hookai company import <path-or-url> --dry-run`
- `hookai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in HOOK storage, or only referenced at import time?
...
3. Should HOOK support direct local folder selection in the web UI, or keep that CLI-only initially?
