# HOOK Memory Service Plan

## Goal

Define a HOOK memory service and surface API that can sit above multiple memory backends, while preserving HOOK's control-plane requirements:

- company scoping
- auditability
- provenance back to HOOK work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current HOOK architecture in:
...
HOOK should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing HOOK plugin runtime.

That keeps the core small and matches the broader HOOK direction that specialized knowledge systems live at the edges.

### 4. HOOK owns routing, provenance, and accounting

Providers should not decide how HOOK entities map to governance.

HOOK core should own:
...
### 5. HOOK exposes shared hooks, providers own extraction

HOOK should emit a common set of memory hooks that built-ins, third-party adapters, and plugins can all use.

Those hooks should pass structured HOOK source objects plus normalized metadata. The provider then decides how to extract from those objects.

HOOK should not force one extraction pipeline or one canonical "memory text" transform before the provider sees the input.
...
HOOK should still log those changes in activity and preserve full auditability. Approval-gated memory governance can remain an enterprise or future policy layer.
...
A mapping from a HOOK target to a binding.
...
The normalized HOOK scope passed into a provider request.
...
A normalized trigger emitted by HOOK when something memory-relevant happens.
...
A normalized write, query, browse, or delete action performed through HOOK.

HOOK should log every operation, whether the provider is local or external.

## Required Adapter Contract

The required core should be small enough to fit `memsearch`, `mem0`, `Memori`, `MemOS`, or `OpenViking`, but strong enough to satisfy HOOK's attribution and inspectability requirements.
...
This contract intentionally does not force a provider to expose its internal graph, file tree, or ontology. It does require enough structure for HOOK to browse, attribute, and audit what happened.
...
That lines up with the HOOK plan at a high level: provider configuration, scoped writes, scoped retrieval, provider-managed extraction as a capability, and a browse and inspect surface.

The concrete changes HOOK should take from AWS are:
...
AWS exposes list and retrieve surfaces directly. HOOK should not make browse optional at the portable layer.
...
HOOK should add cursor-based pagination to:
...
HOOK should keep its own control-plane-centric model, but the adapter contract needs obvious places to map those concepts:
...
AWS exposes extraction jobs explicitly. HOOK should too.
...
### 7. Keep HOOK provenance primary

HOOK should continue to center:
...
## What HOOK Should Persist

HOOK should not mirror the full provider memory corpus into Postgres unless the provider is a HOOK-managed local provider.

HOOK core should persist:
...
HOOK should expose one shared hook system for memory.
...
1. `pre-run hydrate`
   Before an agent run starts, HOOK may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, HOOK may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, HOOK may capture selected issue comments or issue documents as memory sources.

4. `issue_document_capture`
   When enabled on the binding, HOOK may call `capture(...)` for selected issue documents.
...
HOOK should give agents both automatic recall and explicit tools, with simple guidance:
...
HOOK needs a first-class UI for memory, otherwise providers become black boxes.
...
HOOK should treat memory accounting as two related but distinct concerns:
...
- if a memory operation runs inside a normal HOOK agent heartbeat and the model usage is already counted on that run, do not create a duplicate `cost_event`
- instead, store the memory operation with `attributionMode = "included_in_run"` and link it to the related `heartbeatRunId`
- if a memory provider makes a direct metered model call outside the agent run accounting path, the provider must report usage and HOOK should create a `cost_event`
...
HOOK should record evaluation-oriented metrics where possible:
...
This is important because a memory system that "works" but silently burns budget is not acceptable in HOOK.
...
- it matches HOOK's local-first posture
- it is inspectable
...
- Do we want HOOK-managed extraction pipelines at all, or should built-ins be the only place where HOOK owns extraction?
...
- HOOK owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives HOOK a stable "memory service" without locking the product to one memory philosophy or one vendor.
