...
Date: 2026-03-13  
Related discussion: https://github.com/hookai/hook/discussions/449
...
- local runtime data from the default HOOK instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `hook` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `hook` skill currently tells agents to do this on essentially every heartbeat:
...
- HOOK discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing HOOK skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after HOOK-side skill changes land.
...
- `skills/hook/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/hook-create-agent/SKILL.md`: 4,718 bytes
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching HOOK to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `hook` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat HOOK-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `hook` skill rewrite
...
6. Rewrite `skills/hook/SKILL.md` around delta-fetch behavior.
