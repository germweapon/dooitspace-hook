...
Turn the `agent-os` research into a low-risk HOOK execution plan that preserves HOOK's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

HOOK should not absorb `agent-os` as a product model or orchestration layer.

HOOK should evaluate `agent-os` in three narrow areas:
...
This work is successful when HOOK has:
...
- a proposed runtime capability model that fits current HOOK adapters
...
- replace HOOK heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to HOOK core
...
- prove that HOOK can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep HOOK's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a HOOK-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some HOOK workloads
...
- identify where HOOK should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in HOOK
...
- runtime experimentation could blur the boundary between HOOK as control plane and HOOK as execution platform
