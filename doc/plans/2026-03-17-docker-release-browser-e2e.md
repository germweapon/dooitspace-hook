...
Today release smoke testing for published HOOK packages is manual and shell-driven:
...
- runs `hookai@${PAPERCLIPAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@hook.local`
- password: `hook-smoke-password`
...
1. A published `hookai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `hookai@latest` Docker install can be smoke-tested by Playwright in CI.
