...
HOOK's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends HOOK's API, UI, and data model | Yes |
...
modules/
  observability/
    hook.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@hookai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@hookai/core";
...
```
1. Scan modules/ directory for hook.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@hookai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@hookai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:hook/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.hook.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm hookai store list                    # browse available modules and templates
pnpm hookai store install <module-id>     # install a module
pnpm hookai store import <template-id>    # import a company template
pnpm hookai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one HOOK instance | `server:started` |
...
4. **Module config** — `hook.config.json` with per-module config, env var interpolation
...
10. **`@hookai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
