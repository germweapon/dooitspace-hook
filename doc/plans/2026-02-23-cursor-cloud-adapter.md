...
This document defines the V1 design for a HOOK adapter that integrates with
...
4. synthesized stdout events for HOOK UI/CLI
...
3. **Agent callback auth to HOOK** uses a bootstrap exchange flow (no long-lived HOOK key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from HOOK endpoints, not full SKILL.md prompt injection.
...
- `hookPublicUrl` (optional override; else `PAPERCLIP_PUBLIC_URL` env)
...
Goal: avoid putting long-lived HOOK credentials in prompt text.

Flow:

1. Before launch/follow-up, HOOK mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. HOOK validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all HOOK API calls.
...
1. Prompt includes a compact instruction to fetch skills from HOOK.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/hook`
   - `GET /api/skills/hook-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <hookPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active HOOK run context.
...
5. `hookPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from HOOK skill endpoints |
| HOOK auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from HOOK context.
3. Add richer PR metadata surfacing in HOOK UI.
