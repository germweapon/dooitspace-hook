...
This guide describes the current, implemented way to create a HOOK plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main HOOK app.
...
```bash
pnpm --filter @hookai/create-hook-plugin build
node packages/plugins/create-hook-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the HOOK repo:

```bash
pnpm --filter @hookai/create-hook-plugin build
node packages/plugins/create-hook-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/hook/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@hookai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@hookai/plugin-sdk` from the local HOOK checkout into a `.hook-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into HOOK from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@hookai/plugin-sdk/ui`
