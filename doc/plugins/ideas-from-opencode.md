...
HOOK V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a HOOK-shaped design.

Assumption for this document: HOOK is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class HOOK objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into HOOK.

The main conclusion is:

- HOOK should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- HOOK should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- HOOK should use multiple extension classes instead of one generic plugin bag:
...
If HOOK does this well, the examples you listed become straightforward:
...
Relevant HOOK files reviewed for current extension seams:
...
It is too dangerous for HOOK core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for HOOK — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
HOOK should absolutely do this.
...
HOOK should copy this discipline.
...
HOOK should adopt this style for plugin actions, automations, and UI schemas.
...
HOOK can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for HOOK too.

## What HOOK Should Not Copy Directly
...
HOOK is a control plane for an operator-managed instance with company objects.
...
HOOK is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

HOOK should not auto-load arbitrary code from a workspace repo like `.hook/plugins` or project directories.
...
For HOOK, equivalent hooks into:
...
HOOK should not allow plugins to silently replace:
...
For HOOK it would be risky because it combines:
...
HOOK should require an explicit operator install step.

## Why HOOK Needs A Different Shape
...
| Trust assumption | local power user on own machine | operator managing one trusted HOOK instance |
...
That means HOOK should borrow the good ideas from `opencode` but use a stricter architecture.

## HOOK Already Has Useful Pre-Plugin Seams

HOOK has several extension-like seams already:
...
This is good news.
HOOK does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended HOOK Plugin Model
...
HOOK already has this pattern implicitly:
...
- `plugin` means integration code that talks to HOOK through a typed plugin protocol and capability model
...
- invoke explicit HOOK actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@hookai/plugin-sdk/ui`) with design tokens and common components.
...
HOOK should treat plugin installation as a global instance-level action.

Examples:

- install `@hook/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over HOOK objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

HOOK already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for HOOK too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@hook/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@hookai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running HOOK instance.

A `create-hook-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how HOOK achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the HOOK server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. HOOK's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@hookai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@hookai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For HOOK

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@hookai/plugin-sdk";

export default definePlugin({
  id: "@hook/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit HOOK entities
    });
    ...
    ctx.events.on("plugin.@hook/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@hookai/plugin-sdk/ui";
...
Any HOOK plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific HOOK objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@hook/plugin-linear`)
...
- reuse core tables when the data is clearly part of HOOK itself
...
If a concept is becoming part of HOOK's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of HOOK itself
...
### 3. Scope plugin data to HOOK objects before adding custom schemas

A lot of plugin data naturally hangs off existing HOOK objects:
...
If HOOK eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@hook/plugin-workspace-files`
...
Package idea: `@hook/plugin-terminal`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving HOOK.
- Board reviews the diff after a run without leaving HOOK.
...
- optional `events.emit` (e.g. `plugin.@hook/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@hook/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@hook/plugin-linear`

This plugin syncs HOOK work with Linear. It is useful for:

- importing backlog from Linear
- linking HOOK issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between HOOK and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into HOOK.
- HOOK issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@hook/plugin-github-issues`

This plugin syncs HOOK issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to HOOK issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with HOOK task governance
...
- Plugin settings:
  ...
  - whether new HOOK issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active HOOK issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into HOOK.
- GitHub webhooks update status/comment state in HOOK.
- A PR is linked back to the HOOK issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@hook/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@hook/plugin-grafana`

This plugin surfaces external metrics and dashboards inside HOOK. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating HOOK issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full HOOK plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the HOOK dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a HOOK issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of HOOK core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@hook/plugin-runtime-processes`
...
Package idea: `@hook/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into HOOK. It is useful for:
...
- Failed payment spikes or churn events can generate HOOK issues for follow-up.
...
- Stripe data should stay additive to HOOK core
...
- plugin UI bundle loading, host bridge, `@hookai/plugin-sdk/ui`
...
- `@hookai/plugin-test-harness` and `create-hook-plugin` starter template
...
HOOK should not implement "an OpenCode-style generic in-process hook system."
HOOK should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In HOOK
...
8. Ship `@hookai/plugin-test-harness` and `create-hook-plugin` starter template.
