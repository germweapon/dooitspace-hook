# HOOK Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for HOOK's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main HOOK app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary HOOK HTTP APIs directly.
...
HOOK plugin design is based on the following assumptions:

1. HOOK is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core HOOK business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by HOOK core.
...
2. Let plugins add major capabilities without editing HOOK core.
...
4. Depend on project-local plugin folders such as `.hook/plugins`.
...
The single HOOK deployment an operator installs and controls.

### 5.2 Company

A first-class HOOK business object inside the instance.
...
A trusted in-process extension loaded directly by HOOK core.
...
An installable instance-wide extension package loaded through the HOOK plugin runtime.
...
HOOK has two extension classes.
...
HOOK already has a concrete workspace model:
...
Plugins live under the HOOK instance directory.

Suggested layout:

- `~/.hook/instances/default/plugins/package.json`
- `~/.hook/instances/default/plugins/node_modules/`
- `~/.hook/instances/default/plugins/.cache/`
- `~/.hook/instances/default/data/plugins/<plugin-id>/`
...
HOOK should add CLI commands:

- `pnpm hookai plugin list`
- `pnpm hookai plugin install <package[@version]>`
- `pnpm hookai plugin uninstall <plugin-id>`
- `pnpm hookai plugin upgrade <plugin-id> [version]`
- `pnpm hookai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@hook/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface HOOKPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that HOOK agents can use during runs.
...
- HOOK server starts one worker process per installed plugin
...
Receives one typed HOOK domain event.
...
export function definePlugin(definition: PluginDefinition): HOOKPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: HOOKPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@hookai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@hookai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@hookai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@hookai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@hookai/plugin-sdk/ui`
...
The `@hookai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the HOOK secret provider system rather than a plain text input
...
1. Core HOOK data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing HOOK objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual HOOK product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if HOOK later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing HOOK secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm hookai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the HOOK server. This is a normative requirement, not optional.
...
### 27.1 `@hookai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@hookai/plugin-test-harness";
...
For developing a plugin against a running HOOK instance:

- The operator installs the plugin from a local path: `pnpm hookai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-hook-plugin`) that scaffolds:

- `package.json` with correct `hookPlugin` keys
...
- `@hook/plugin-workspace-files`
- `@hook/plugin-terminal`
- `@hook/plugin-git`
- `@hook/plugin-linear`
- `@hook/plugin-github-issues`
- `@hook/plugin-grafana`
- `@hook/plugin-runtime-processes`
- `@hook/plugin-stripe`
...
- `@hookai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@hookai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@hookai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@hookai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@hookai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@hookai/plugin-sdk/ui`
...
- `@hookai/plugin-test-harness`
- `create-hook-plugin` starter template
...
HOOK should not implement a generic in-process hook bag modeled directly after local coding tools.
...
That is the complete target design for the HOOK plugin system.
