...
This spec defines how HOOK actually runs agents while staying runtime-agnostic.
...
1. HOOK is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, HOOK manages lifecycle, and on exit HOOK parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `hook.skill` (shared HOOK skill text block)
