...
For `claude_local` and `codex_local`, HOOK assumes the CLI is already installed and authenticated on the host machine.
...
HOOK stores resumable session state per `(agent, taskKey, adapterType)`.
