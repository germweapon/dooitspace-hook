...
ClipHub is the public registry where people share, discover, and download HOOK company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to HOOK what a package registry is to a programming language. HOOK already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in HOOK — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own HOOK instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the HOOK spec. It contains:
...
**Install (fresh start):**
```
hook install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local HOOK instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within HOOK, export your company as a template, then publish:

```
hook export --template my-company
hook publish cliphub my-company
```
...
ClipHub is a **separate service** from HOOK itself. HOOK is self-hosted; ClipHub is a hosted registry that HOOK instances talk to.
...
| **HOOK CLI** | `hookai install`, `hookai publish`, `hookai cliphub sync` — built into HOOK |
| **HOOK UI** | "Browse ClipHub" panel in the HOOK web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with HOOK) |
| Backend | TypeScript + Hono (consistent with HOOK) |
| Database | PostgreSQL |
...
4. Run `hookai install cliphub:acme/lean-saas-shop`
5. HOOK creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in HOOK until it works well
2. Export: `hookai export --template my-agency`
3. Publish: `hookai publish cliphub my-agency`
...
3. Install just that agent: `hookai install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use HOOK. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to HOOK what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `hookai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in HOOK web UI
- [ ] `hookai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with HOOK's own philosophy)
