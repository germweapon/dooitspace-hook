...
Low-level reference for how HOOK packages are prepared and published to npm.
...
HOOK no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `hookai`, imports code from workspace packages such as:
...
### Manual first publish for `@hookai/ui`
...
npm view @hookai/ui version

# make sure the dist payload is fresh
pnpm --filter @hookai/ui build
...
- If `npm view @hookai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@hookai/ui`, the `hookai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
- `hookai@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx hookai@canary onboard
```
