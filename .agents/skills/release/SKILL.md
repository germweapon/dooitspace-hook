...
name: release
description: >
  Coordinate a full HOOK release across engineering verification, npm,
...
Run the full HOOK maintainer release workflow, not just an npm publish.
...
7. If running through HOOK, you have issue context for status updates and follow-up task creation.
...
HOOK now uses a commit-driven release model:
...
npm view hookai@canary version
```
...
- release summary in HOOK issue context
