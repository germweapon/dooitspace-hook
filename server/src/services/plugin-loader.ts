...
/**
 * Naming convention for npm-published HOOK plugins.
 * Packages matching this pattern are considered HOOK plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
...
export interface DiscoveredPlugin {
  ...
  manifest: HOOKPluginManifestV1 | null;
}
...
function getDeclaredPageRoutePaths(manifest: HOOKPluginManifestV1): string[] {
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: HOOKPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover HOOK plugins installed as npm packages in the current
   ...
   *
   * Returns null if the package is not a HOOK plugin.
   * Throws if the package is a HOOK plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<HOOKPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: HOOKPluginManifestV1;
    newManifest: HOOKPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the HOOK plugin naming convention.
...
function getMinimumHostVersion(manifest: HOOKPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumHOOKVersion;
}
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: HOOKPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as HOOKPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a HOOK plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a HOOK plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasHOOKPlugin = "paperclipPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasHOOKPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<HOOKPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: HOOKPluginManifestV1;
      } | null;
