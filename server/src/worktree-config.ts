...
import type { HOOKConfig } from "@paperclipai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by HOOK worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  const configPath = resolveHOOKConfigPath(overrideConfigPath);
...
function writeConfigFile(configPath: string, config: HOOKConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as HOOKConfig;
...
): { config: HOOKConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as HOOKConfig;
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: HOOKConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as HOOKConfig;
  } catch {
    return;
  }
