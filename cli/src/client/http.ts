...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the HOOK server is not running, the configured URL is wrong, or the request is being blocked before it reaches HOOK.",
    "",
    "Try:",
    "- Start HOOK with `pnpm dev` or `pnpm paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If HOOK is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
