...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by HOOK CLI commands",
...
export function resolveHOOKEnvFile(configPath?: string): string {
...
export function loadHOOKEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeHOOKEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readHOOKEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeHOOKEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeHOOKEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readHOOKEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeHOOKEnvEntries(next, filePath);
  return next;
}
