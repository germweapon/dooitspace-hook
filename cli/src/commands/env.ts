...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveHOOKInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveHOOKInstanceId());
}
...
function collectDeploymentEnvRows(config: HOOKConfig | null, configPath: string): EnvVarRow[] {
