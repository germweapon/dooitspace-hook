...
OPENCLAW_USER="${OPENCLAW_USER:-hook-smoke}"
...
Run your HOOK heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  '{
    ...
    metadata: {
      ...
      hook_session_key: ("hook:run:" + $runId)
    }
  }')"
...
args=(
  ...
  -H "x-openclaw-session-key: hook:run:${PAPERCLIP_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
