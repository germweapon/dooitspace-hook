...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.hook-openclaw.override.yml"
...
hook_base_url="$(detect_hook_base_url || true)"
...
HOOK URL for OpenClaw container:
EOF
...
EOF
  ...
  If HOOK rejects the host, run on host machine:
    pnpm hookai allowed-hostname host.docker.internal
  Then restart HOOK and re-run this script.
EOF
  fi
  cat <<EOF
