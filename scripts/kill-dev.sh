#!/usr/bin/env bash
#
# Kill all local HOOK dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all hook dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/hook(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.hook/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.hook/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.hook/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/hook*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.hook/instances/*/db/postmaster.pid
    "$sibling_root"/.hook/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No HOOK dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} HOOK dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to HOOK node processes..."
