...
const DEFAULT_QUERY = "\"Co-Authored-By: HOOK <noreply@hook.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/hook-commit-metrics-cache.json");
...
const PAPERCLIP_EMAIL = "noreply@hook.ing";
const PAPERCLIP_NAME = "hook";
...
function printHelp() {
  console.log(`Usage: tsx scripts/hook-commit-metrics.ts [options]
...
function printSummary(summary: Summary) {
  console.log("HOOK commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "hook-commit-metrics",
