...
Environment overrides:
  ...
  PC_DATA=/tmp/data           HOOK data dir to use
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/hook-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/hook-data}"
...
env \
  ...
  pnpm hookai onboard --yes --data-dir "$PC_DATA"
