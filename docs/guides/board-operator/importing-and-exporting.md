...
HOOK companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .hook.yaml     # Adapter config, env inputs, routines
```
...
- **.hook.yaml** holds HOOK-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
hookai company export <company-id> --out ./my-export
```
...
hookai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
hookai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
hookai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.hook.yaml`
...
```sh
# From a local folder
hookai company import ./my-export

# From a GitHub URL
hookai company import https://github.com/org/repo

# From a GitHub subfolder
hookai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
hookai company import org/repo
hookai company import org/repo/companies/acme
```
...
If `--target` is not specified, HOOK infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
hookai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
hookai company import org/company-templates/engineering-team \
  --target new \
...
```sh
hookai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
hookai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
hookai company import ./package \
  --target new \
  --yes \
  --json
```
...
HOOK supports several GitHub URL formats:
