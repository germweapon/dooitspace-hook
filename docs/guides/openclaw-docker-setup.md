...
How to get OpenClaw running in a Docker container for local development and testing the HOOK OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

HOOK includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-hook-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a HOOK host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-hook-smoke`)
...
If your HOOK deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host HOOK server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed HOOK URL (typically `http://host.docker.internal:3100`).
- If HOOK rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm hookai allowed-hostname host.docker.internal
```

Then restart HOOK and rerun the smoke script.
...
```bash
pnpm hookai allowed-hostname <host>
```
