# ClipHub: Marketplace for HOOK Team Configurations
...
> The "app store" for whole-company AI teams — pre-built HOOK configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for HOOK-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | HOOK company (agents, projects, governance) |
...
A complete HOOK company configuration:
...
Single-agent configurations designed to plug into a HOOK org:
...
Portable skill files that any HOOK agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with HOOK's skill loading system
...
interface Listing {
  ...
  hookVersionMin: string;     // Minimum HOOK version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target HOOK company
...
| `POST` | `/api/listings/:id/install` | Install to HOOK company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target HOOK company (or create new)
  → ClipHub API calls HOOK API to:
  ...
  → Redirect to HOOK dashboard with new team running
```
...
### 5.3 Creator: Export from HOOK → Publish

```
Running HOOK company → "Export as Blueprint" (CLI or UI)
  → HOOK exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (HOOK brand blue/purple)
...
| Install flow | One-click deploy to HOOK company |
...
{
  "targetCompanyId": "uuid",         // Existing HOOK company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `hook-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as HOOK
- **Backend**: Node.js API (or extend HOOK server)
- **Database**: Postgres (can share HOOK's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with HOOK auth (or OAuth2)

### 9.2 Integration with HOOK

ClipHub can be:
- **Option A**: A separate app that calls HOOK's API to install blueprints
- **Option B**: A built-in section of the HOOK UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing HOOK UI and API.
...
- [ ] Install flow: blueprint → HOOK company
