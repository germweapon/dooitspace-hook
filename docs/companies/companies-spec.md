...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only HOOK.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.hook.yaml`
...
- HOOK must not require extra top-level fields for skill validity
- HOOK-specific extensions must live under `metadata.hook` or `metadata.sources`
...
For HOOK, the preferred fidelity extension is:
...
- HOOK tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean
...
Additional rules for HOOK exporters:
...
- omit empty and default-valued HOOK fields when possible
...
- HOOK exporters should emit `.hook.yaml` as a sidecar by default
...
HOOK can map this spec to its runtime model like this:
...
- HOOK extension:
  - `.hook.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other HOOK-specific fidelity

Inline HOOK-only metadata that must live inside a shared markdown file should use:

- `metadata.hook`

That keeps the base format broader than HOOK.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only HOOK.

## 20. Cutover

HOOK should cut over to this markdown-first package model as the primary portability format.

`hook.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For HOOK, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
