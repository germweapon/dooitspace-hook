...
Build a custom adapter to connect HOOK to any agent runtime.
...
|---|---|---|
| Source | Inside `hook-fork` | Separate npm package |
| Distribution | Ships with HOOK | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying HOOK's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@hookai/adapter-utils/server-utils`
2. Build environment with `buildHOOKEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@hookai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildHOOKEnv(agent)` | `@hookai/adapter-utils/server-utils` | Inject `PAPERCLIP_*` env vars |
| `renderTemplate(tpl, data)` | `@hookai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@hookai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@hookai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in HOOK's source:
...
`format-event.ts` — pretty-prints stdout for `hookai run --watch` using `picocolors`.
...
Make HOOK skills discoverable to your agent runtime without writing to the agent's working directory:
