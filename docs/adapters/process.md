...
- Running a Python script that calls the HOOK API
...
1. HOOK spawns the configured command as a child process
2. Standard HOOK environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the HOOK API and perform work.
