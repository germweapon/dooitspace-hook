...
summary: Build, package, and distribute adapters as plugins without modifying HOOK source
---

HOOK supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to HOOK's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `hook-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with HOOK | Published to npm or linked via `file:` |
| Updates | Requires HOOK release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@hookai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `hook.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@hookai/adapter-utils";
...
} from "@hookai/adapter-utils";

import {
  runChildProcess,
  buildHOOKEnv,
  renderTemplate,
} from "@hookai/adapter-utils/server-utils";
...
export async function execute(
  ...
  // 2. Build environment with HOOK vars injected
  const env = buildHOOKEnv(agent);
...
#### Available Helpers from `@hookai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildHOOKEnv(agent)` | Inject `PAPERCLIP_*` environment variables |
...
```sh
# Via the HOOK UI
# Settings → Adapters → Install from npm → "my-hook-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-hook-adapter"}'
```
...
Local adapters are symlinked into HOOK's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.hook/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-hook-adapter",
...
```ts
import type { AdapterSessionCodec } from "@hookai/adapter-utils";
...
Other HOOK users can then install your adapter by package name from the UI or API.
