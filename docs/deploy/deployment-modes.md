...
HOOK supports two runtime modes with different security profiles.
...
```sh
pnpm hookai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, HOOK emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated pnpm hookai run
```
