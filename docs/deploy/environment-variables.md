...
All environment variables that HOOK uses for server configuration.
...
| `PAPERCLIP_HOME` | `~/.hook` | Base directory for all HOOK data |
...
| `PAPERCLIP_SECRETS_MASTER_KEY_FILE` | `~/.hook/.../secrets/master.key` | Path to key file |
...
| `PAPERCLIP_API_URL` | HOOK API base URL |
