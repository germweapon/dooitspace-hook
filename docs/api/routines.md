...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-HOOK-Signature` + `X-HOOK-Timestamp` header pair matching the trigger's signing mode.
