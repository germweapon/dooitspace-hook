...
Add first-class **documents** to HOOK as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by HOOK workflow/docs.
...
Update the HOOK agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/hook/SKILL.md`
