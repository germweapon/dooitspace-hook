...
function resolveHOOKHomeDir(): string {
  const envHome = process.env.PAPERCLIP_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".paperclip");
}

function resolveHOOKInstanceId(): string {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveHOOKHomeDir(), "instances", resolveHOOKInstanceId(), "db");
}
...
function resolveHOOKConfigPath(): string {
...
function resolveHOOKEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveHOOKEnvPath(configPath);
