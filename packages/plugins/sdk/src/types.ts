/**
 * Core types for the HOOK plugin worker-side SDK.
 *
...
export interface ScopeKey {
  /** What kind of HOOK object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the HOOK UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit HOOK domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core HOOK domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the HOOK secret provider
...
export interface PluginSecretsClient {
   ...
   * in the HOOK secret provider (e.g. `"MY_API_KEY"`).
   *
