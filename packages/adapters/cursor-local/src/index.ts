...
Use when:
- You want HOOK to run Cursor Agent CLI locally as the agent runtime
...
- HOOK auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- HOOK auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
