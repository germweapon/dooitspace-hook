...
function renderHOOKEnvNote(env: Record<string, string>): string {
...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    "HOOK API access note:",
    "Use run_shell_command with curl to make HOOK API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $PAPERCLIP_API_KEY\\" \\"$PAPERCLIP_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $PAPERCLIP_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-HOOK-Run-Id: $PAPERCLIP_RUN_ID\\" -d '{...}' \\"$PAPERCLIP_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject HOOK skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      const result = await ensureHOOKSkillSymlink(entry.source, target);
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readHOOKRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveHOOKDesiredSkillNames(config, geminiSkillEntries);
  ...
  const paperclipEnvNote = renderHOOKEnvNote(env);
