...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readHOOKRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveHOOKDesiredSkillNames(config, piSkillEntries);
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderHOOKWakePrompt(context.paperclipWake, { resumedSession: canResumeSession });
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add HOOK skills directory so Pi can load the paperclip skill
