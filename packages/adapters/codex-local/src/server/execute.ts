...
import {
  ...
  ensureHOOKSkillSymlink,
  ensurePathInEnv,
  readHOOKRuntimeSkillEntries,
  resolveCommandForLogs,
  resolveHOOKDesiredSkillNames,
  renderTemplate,
...
async function isLikelyHOOKRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelyHOOKRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyHOOKRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableHOOKSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyHOOKRuntimeSkillPath(resolvedLinkedPath, entry.name, {
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readHOOKRuntimeSkillEntries({}, __moduleDir);
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelyHOOKRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
      ...
      const result = await ensureHOOKSkillSymlink(entry.source, target, linkSkill);
  ...
  await pruneBrokenUnavailableHOOKSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readHOOKRuntimeSkillEntries(config, __moduleDir);
  ...
  const repoAgentsNote =
    "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; HOOK does not currently suppress that discovery.";
