...
Use when:
- You want HOOK to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the HOOK server.
...
- paperclipApiUrl (string, optional): absolute HOOK base URL advertised in wake text
...
- paperclip (object): standardized HOOK context added to every gateway agent request
- paperclip.workspace (object, optional): resolved execution workspace for this run
- paperclip.workspaces (array, optional): additional workspace hints HOOK exposed to the run
