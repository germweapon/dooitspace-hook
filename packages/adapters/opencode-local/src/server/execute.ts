...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const openCodeSkillEntries = await readHOOKRuntimeSkillEntries(config, __moduleDir);
  const desiredOpenCodeSkillNames = resolveHOOKDesiredSkillNames(config, openCodeSkillEntries);
