...
async function buildOpenCodeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const desiredSkills = resolveHOOKDesiredSkillNames(config, availableEntries);
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside HOOK management in the shared skills home.",
    warnings: [
...
export async function syncOpenCodeSkills(
  ...
  const availableEntries = await readHOOKRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureHOOKSkillSymlink(available.source, target);
  }
...
export function resolveOpenCodeDesiredSkillNames(
  ...
  return resolveHOOKDesiredSkillNames(config, availableEntries);
}
