...
async function buildClaudeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const entries: AdapterSkillEntry[] = availableEntries.map((entry) => ({
    ...
    originLabel: entry.required ? "Required by HOOK" : "Managed by HOOK",
    readOnly: false,
    sourcePath: entry.source,
    targetPath: null,
    detail: desiredSet.has(entry.key)
      ? "Will be materialized into the stable HOOK-managed Claude prompt bundle on the next run."
      : null,
  ...
  for (const [name, installedEntry] of installed.entries()) {
    ...
    entries.push({
      ...
      detail: "Installed outside HOOK management in the Claude skills home.",
    });
  }
