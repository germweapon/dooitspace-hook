 ...
 * renderYamlBlock.
 */
function filterHOOKYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("```");
  lines.push("");
  lines.push("See [HOOK](https://paperclip.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [HOOK](https://paperclip.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  const effectiveFiles = useMemo(() => {
    ...
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterHOOKYaml(exportData.files[yamlPath], checkedFiles);
    }
