---
name: hook-create-plugin
description: >
  Create new HOOK plugins with the current alpha SDK/runtime. Use when
...
Use this skill when the task is to create, scaffold, or document a HOOK plugin.
...
node packages/plugins/create-hook-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the HOOK repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.hook-sdk/`:

```bash
pnpm --filter @hookai/create-hook-plugin build
node packages/plugins/create-hook-plugin/dist/index.js @acme/plugin-name \
...
- is installed into HOOK from an absolute local path during development
